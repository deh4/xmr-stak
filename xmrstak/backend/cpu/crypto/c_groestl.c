...
inline void F1024(u64 *h, const u64 *m) {
  static u64 y[COLS1024] ALIGN(16);
  static u64 z[COLS1024] ALIGN(16);
  static u64 outQ[COLS1024] ALIGN(16);
  static u64 inP[COLS1024] ALIGN(16);
  int i;
...
/* initialise context */
HashReturnGroest Init(hashState* ctx,
		int hashbitlen) {
...
HashReturnGroest Update(hashState* ctx,
...
HashReturnGroest Final(hashState* ctx,
		 BitSequence* output) {
...
/* hash bit sequence */
HashReturnGroest groestlHash(int hashbitlen,
		const BitSequence* data, 
		DataLength databitlen,
		BitSequence* hashval) {
	HashReturnGroest ret;
  hashState context;
